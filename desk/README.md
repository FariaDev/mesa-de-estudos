...
- **Citação que abre o material**: `Limites.pdf, p. 7` na resposta do Pi é clicável — abre o PDF naquela página (ou carrega o arquivo no leitor). O **Voltar** da cabeça do leitor desfaz o pulo; o botão **Navegar** tem os **favoritos nomeados** da matéria e o **sumário** do documento.
- **Caderno de revisão**: em toda resposta do Pi, **Guardar para revisar** abre o diálogo já preenchido (questão da Lista ativa, sua última tentativa, o trecho selecionado como dificuldade e a página aberta como referência). O caderno abre no menu **Estudar → Caderno de revisão**, em aba (o mesmo canto do GeoGebra); cada item oferece **Semelhante** e **Tentar de novo** (escrevem o pedido no composer), editar (clique no título) e **Remover** (com confirmação).
- **Ajustes e modo compacto**: Modelo, Esforço e compactação vivem no painel recolhível **Ajustes**, no topo da conversa; **Encerrar por hoje** fica no menu **Estudar**. O **Modo compacto** (menu **Mesa** ou **⌘⇧D**) aperta os espaçamentos.
...
- Digite uma pergunta e envie com **⏎** (**⇧⏎** quebra linha). Com o Pi trabalhando, **⏎** enfileira a próxima mensagem — a faixa acima do campo deixa editar ou tirar da fila antes da vez — e **⌘⏎** interrompe e envia na hora. A fila fica salva no disco, por conversa e com os anexos: fechar e reabrir a mesa devolve os itens na faixa, marcados como **Recuperadas** e segurados — nada sai sozinho; **Enviar agora** libera e **Limpar** descarta (com confirmação). O seletor “Incluir referências” acrescenta caminhos e páginas abertos como contexto. Os conteúdos não são enviados em toda pergunta: Pi consulta os arquivos quando necessário.
...
- Anexe imagens ao prompt pelo botão **Anexar**, colando (⌘V em qualquer ponto da mesa) ou arrastando para o chat. A miniatura fica acima do campo (clique nela para ver a imagem grande, como nas imagens da conversa) e pode ser removida antes do envio. Modelos sem visão recusam o anexo. A página atual de um PDF vai como imagem com o botão de câmera no título do leitor. Anexos acima de 1568 px por lado são reduzidos na hora, antes de entrar na fila (o GIF é enviado como veio).
...
- **Encerrar por hoje** grava o registro local da matéria primeiro (onde você parou, o próximo passo, a questão, o `.xopp` e as páginas) e só então avisa o Pi — sem Pi o registro fica igual. Na abertura seguinte o cartão **Continuar …** fica acima do campo: **Retomar** restaura a questão, a página e o `.xopp`, pré-preenche a mensagem e apaga o registro; **Dispensar** só apaga.
...
- **Parar** interrompe a resposta e **segura** a fila — a faixa troca para **Enviar agora** / **Limpar** e nenhum item se perde. `＋` na conversa começa outra sessão, preservando o JSONL anterior.
- A calculadora aceita `+ - * / ^`, parênteses, `sqrt`, `abs`, `sin`, `cos`, `tan`, `sec`, `csc`, `cot`, `asin`, `acos`, `atan`, `ln`, `log`, `exp`, `pi` e `e`. Multiplicação é explícita: `2*pi`. RAD/GRAUS muda as funções trigonométricas e as inversas devolvem o ângulo na unidade escolhida (`asin(0.5)` dá `30` em GRAUS). Ângulos exatos saem exatos (`cos(90)` é `0`) e os polos ficam indefinidos (`tan(90)` vira `—` com aviso). O histórico permite reutilizar expressões.

## Dados e integração

Fontes são abertas somente para leitura. O estado dos PDFs e a conversa ficam em `desk/.runtime/`. A fila do composer (itens e anexos) e o registro do Encerrar por hoje também: `.runtime/pending.json`, por conversa, e `.runtime/resume.json`, por matéria. A sessão Pi é um JSONL próprio; não assume uma sessão já aberta no terminal. Para exibir gráficos no chat, a mesa lê imagens somente dentro da área de aprendizado em `.runtime`; caminhos fora dela são ignorados.
...
**Rodada rápida.** O `npm test` tem portão com cache: `build:bend` + `test:proof`
+ `test:parity` só rodam quando muda o conteúdo que entra neles — `core/**`, os
artefatos gerados, os verificadores de paridade (com os hosts que eles importam)
e o `package.json` —, não a cada edição de host
(`MESA_GATE_CACHE=0 npm test` força a rodada inteira; o portão de release,
`npm run verify:bend`, nunca usa cache). Os E2E sobem o Electron, então rodam em
paralelo: `npm run hunts` (2 por vez; `npm run hunts fila`, `--jobs 4`,
`--filter <cenário>`), e `node tests/ui-smoke.mjs` segue sendo a rodada completa.
