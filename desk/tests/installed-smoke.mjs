...
const fakePi=path.resolve('tests','fake-pi.mjs');
try{app=await electron.launch({executablePath:executable,args:[],env:testEnv({LEARNING_DESK_RUNTIME:runtime,LEARNING_DESK_PI:fakePi})});const p=await app.firstWindow();
 ...
 app=await electron.launch({executablePath:executable,args:[],env:testEnv({LEARNING_DESK_RUNTIME:runtime,LEARNING_DESK_PI:fakePi})});const restored=await app.firstWindow();await restored.waitForFunction(()=>document.querySelector('.pdf-foot')?.textContent.includes('Página 3'));
 ...
 console.log('PACKAGED APP PASSED: actual bundle, PDFs, calculator, page/zoom restoration.');
