// GERADO de core/toastview.bend por `bun core/build.mjs` — não editar à mão.
// core/toastview.bend
...
function $toastClasses$(closing_0) {
  return { $: "Con", ["head"]: run_loop($view$classOn$("toast-item", true)), ["tail"]: { $: "Con", ["head"]: run_loop($view$classOn$("closing", closing_0)), ["tail"]: { $: "Con", ["head"]: run_loop($view$classOn$("out", closing_0)), ["tail"]: { $: "Nil" } } } };
}
function $toastItem$(t_0) {
  const text_0 = t_0.text;
  const closing_0 = t_0.closing;
  return run_jump($view$viewEl$, ["div", { $: "Con", ["head"]: run_loop($view$classes$(run_loop($toastClasses$(closing_0)))), ["tail"]: { $: "Nil" } }, { $: "Con", ["head"]: run_loop($view$viewText$(text_0)), ["tail"]: { $: "Nil" } }]);
}
function $view$viewMap$1$(xs_0) {
  if (xs_0.$ === "Nil") {
    ...
    const h_0 = xs_0.head;
    const t_0 = xs_0.tail;
    return { $: "Con", ["head"]: run_loop($toastItem$(h_0)), ["tail"]: run_loop($view$viewMap$1$(t_0)) };
  }
}
function $toastItems$(ts_0) {
  return run_jump($view$viewMap$1$, [ts_0]);
}
function $toastBox$(ts_0) {
  return run_jump($view$viewEl$, ["div", { $: "Con", ["head"]: run_loop($view$attrId$("toast")), ["tail"]: { $: "Con", ["head"]: run_loop($view$attr$("role", "status")), ["tail"]: { $: "Nil" } } }, run_loop($toastItems$(ts_0))]);
}
function $pulseDot$() {
  return run_jump($view$viewEl$, ["span", { $: "Con", ["head"]: run_loop($view$attrClass$("pulse")), ["tail"]: { $: "Con", ["head"]: run_loop($view$attrAria$("hidden", "true")), ["tail"]: { $: "Nil" } } }, { $: "Nil" }]);
}
function $activityLabel$(text_0) {
  return run_jump($view$viewEl$, ["span", { $: "Con", ["head"]: run_loop($view$attrClass$("activity-label")), ["tail"]: { $: "Nil" } }, { $: "Con", ["head"]: run_loop($view$viewText$(text_0)), ["tail"]: { $: "Nil" } }]);
}
function $activityText$body$(text_0, empty_0) {
  if (empty_0) {
    return { $: "Nil" };
  } else {
...
function $activityText$(text_0) {
  return run_jump($activityText$body$, [text_0, run_loop($String$is_empty$(text_0))]);
}
function $activityDot$body$(label_0, empty_0) {
  if (empty_0) {
    return { $: "Nil" };
  } else {
    return { $: "Con", ["head"]: run_loop($pulseDot$()), ["tail"]: { $: "Con", ["head"]: run_loop($activityLabel$(label_0)), ["tail"]: { $: "Nil" } } };
  }
}
function $activityDot$(label_0) {
  return run_jump($activityDot$body$, [label_0, run_loop($String$is_empty$(label_0))]);
}
function $activityNode$(title_0, kids_0) {
  return run_jump($view$viewEl$, ["div", { $: "Con", ["head"]: run_loop($view$attrId$("activity")), ["tail"]: { $: "Con", ["head"]: run_loop($view$attr$("role", "status")), ["tail"]: { $: "Con", ["head"]: run_loop($view$attrTitle$(title_0)), ["tail"]: { $: "Nil" } } } }, kids_0]);
}
function $activityView$(text_0) {
  return run_jump($activityNode$, [text_0, run_loop($activityText$(text_0))]);
}
function $activityLiveView$(text_0) {
  return run_jump($activityNode$, [text_0, run_loop($activityDot$(text_0))]);
}
...
var toastview_default = {
  ...
  toastClasses: run_lib($toastClasses$, 1),
  toastItem: run_lib($toastItem$, 1),
  "view.viewMap~1": run_lib($view$viewMap$1$, 1),
  toastItems: run_lib($toastItems$, 1),
  toastBox: run_lib($toastBox$, 1),
  pulseDot: run_lib($pulseDot$, 0),
  activityLabel: run_lib($activityLabel$, 1),
  "activityText.body": run_lib($activityText$body$, 2),
  activityText: run_lib($activityText$, 1),
  "activityDot.body": run_lib($activityDot$body$, 2),
  activityDot: run_lib($activityDot$, 1),
  activityNode: run_lib($activityNode$, 2),
  activityView: run_lib($activityView$, 1),
  activityLiveView: run_lib($activityLiveView$, 1)
};
export {
  toastview_default as default
};
