...
function $densityValue$if$(_compact_0) {
  if (_compact_0) {
    return "compacta";
  } else {
    return "padrao";
...
function $densityValue$(_raw_0) {
  return run_jump($densityValue$if$, [run_loop($String$eq$(_raw_0, "compacta"))]);
}
function $densityDense$(_mode_0) {
  return run_jump($String$eq$, [_mode_0, "compacta"]);
}
function $densityOption$(_value_0, _label_0, _selected_0) {
  return run_jump($view$viewEl$, ["option", run_loop($view$attrConcat$({ $: "Con", ["head"]: { $: "ViewAttr", ["name"]: "value", ["value"]: _value_0 }, ["tail"]: { $: "Nil" } }, run_loop($view$attrWhen$(_selected_0, { $: "ViewAttr", ["name"]: "selected", ["value"]: "" })))), { $: "Con", ["head"]: run_loop($view$viewText$(_label_0)), ["tail"]: { $: "Nil" } }]);
}
function $densitySelect$(_mode_0) {
  return run_jump($view$viewEl$, ["select", { $: "Con", ["head"]: run_loop($view$attrId$("density-mode")), ["tail"]: { $: "Con", ["head"]: run_loop($view$attrOn$("change", "SetDensity")), ["tail"]: { $: "Nil" } } }, { $: "Con", ["head"]: run_loop($densityOption$("padrao", "Padrão", run_loop($String$eq$(_mode_0, "padrao")))), ["tail"]: { $: "Con", ["head"]: run_loop($densityOption$("compacta", "Compacta", run_loop($String$eq$(_mode_0, "compacta")))), ["tail"]: { $: "Nil" } } }]);
}
function $densityFields$(_mode_0) {
  return { $: "Con", ["head"]: run_loop($view$viewText$("Densidade")), ["tail"]: { $: "Con", ["head"]: run_loop($densitySelect$(_mode_0)), ["tail"]: { $: "Nil" } } };
}
function $densityControl$(_mode_0) {
  return run_jump($view$viewEl$, ["label", { $: "Nil" }, run_loop($densityFields$(_mode_0))]);
}
function $tooltipClasses$(_below_0) {
  return { $: "Con", ["head"]: run_loop($view$classOn$("tip", true)), ["tail"]: { $: "Con", ["head"]: run_loop($view$classOn$("below", _below_0)), ["tail"]: { $: "Nil" } } };
}
function $tooltipView$(_open_0, _text_0, _below_0) {
  return run_jump($view$viewEl$, ["div", run_loop($view$attrConcat$({ $: "Con", ["head"]: run_loop($view$classes$(run_loop($tooltipClasses$(_below_0)))), ["tail"]: { $: "Nil" } }, run_loop($view$attrWhen$(run_loop($Bool$not$(_open_0)), run_loop($view$attr$("hidden", "")))))), { $: "Con", ["head"]: run_loop($view$viewText$(_text_0)), ["tail"]: { $: "Nil" } }]);
}
function $checkRow$(_id_0, _label_0, _checked_0) {
  return run_jump($view$viewEl$, ["label", { $: "Con", ["head"]: run_loop($view$attrClass$("set-check")), ["tail"]: { $: "Nil" } }, { $: "Con", ["head"]: run_loop($view$viewEl$("input", run_loop($view$attrConcat$({ $: "Con", ["head"]: run_loop($view$attrType$("checkbox")), ["tail"]: { $: "Con", ["head"]: run_loop($view$attrId$(_id_0)), ["tail"]: { $: "Con", ["head"]: run_loop($view$attrOn$("change", "SetNotify")), ["tail"]: { $: "Nil" } } } }, run_loop($view$attrWhen$(_checked_0, run_loop($view$attr$("checked", "")))))), { $: "Nil" })), ["tail"]: { $: "Con", ["head"]: run_loop($view$viewEl$("span", { $: "Nil" }, { $: "Con", ["head"]: run_loop($view$viewText$(_label_0)), ["tail"]: { $: "Nil" } })), ["tail"]: { $: "Nil" } } }]);
}
...
function $notifyRows$(_prefs_0) {
  const _sound_0 = _prefs_0["sound"];
  const _desktop_0 = _prefs_0["desktop"];
  const _focused_0 = _prefs_0["focused"];
  return { $: "Con", ["head"]: run_loop($checkRow$("notify-sound", "Som", _sound_0)), ["tail"]: { $: "Con", ["head"]: run_loop($checkRow$("notify-desktop", "Notificação", _desktop_0)), ["tail"]: { $: "Con", ["head"]: run_loop($checkRow$("notify-focused", "Avisar mesmo com a janela em foco", _focused_0)), ["tail"]: { $: "Nil" } } } };
}
function $notifyControls$(_prefs_0) {
  return run_jump($List$append$, [{ $: "Con", ["head"]: run_loop($notifyLegend$()), ["tail"]: { $: "Nil" } }, run_loop($notifyRows$(_prefs_0))]);
}
function $notifyGroup$(_prefs_0) {
  return run_jump($view$viewEl$, ["div", { $: "Con", ["head"]: run_loop($view$attrId$("notify-mode")), ["tail"]: { $: "Con", ["head"]: run_loop($view$attrClass$("set-group")), ["tail"]: { $: "Con", ["head"]: run_loop($view$attr$("role", "group")), ["tail"]: { $: "Con", ["head"]: run_loop($view$attrAria$("label", "Avisos de conclusão")), ["tail"]: { $: "Nil" } } } } }, run_loop($notifyControls$(_prefs_0))]);
}
