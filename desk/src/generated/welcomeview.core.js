...
function $welcomeTitle$(t_0, empty_0) {
  if (empty_0) {
    return { $: "Nil" };
  } else {
    return { $: "Con", ["head"]: run_loop($view$viewEl$("h3", { $: "Nil" }, { $: "Con", ["head"]: run_loop($view$viewText$(t_0)), ["tail"]: { $: "Nil" } })), ["tail"]: { $: "Nil" } };
  }
}
...
function $welcomeChildren$(title_0) {
  return run_jump($List$append$, [run_loop($welcomeTitle$(title_0, run_loop($String$is_empty$(title_0)))), { $: "Con", ["head"]: run_loop($welcomeButton$()), ["tail"]: { $: "Con", ["head"]: run_loop($welcomeParagraph$()), ["tail"]: { $: "Nil" } } }]);
}
function $welcomeView$(title_0) {
  return run_jump($view$viewEl$, ["div", { $: "Con", ["head"]: run_loop($view$attrId$("welcome")), ["tail"]: { $: "Nil" } }, run_loop($welcomeChildren$(title_0))]);
}
...
function $chatWelcomeTail$(connected_0, connecting_0) {
  if (connected_0) {
    return { $: "Nil" };
  } else {
    if (connecting_0) {
...
function $chatWelcomeChildren$(connected_0, connecting_0) {
  return run_jump($List$append$, [{ $: "Con", ["head"]: run_loop($chatWelcomeQuestion$()), ["tail"]: { $: "Con", ["head"]: run_loop($chatWelcomeIntro$()), ["tail"]: { $: "Nil" } } }, run_loop($chatWelcomeTail$(connected_0, connecting_0))]);
}
function $chatWelcomeView$(connected_0, connecting_0) {
  return run_jump($view$viewEl$, ["div", { $: "Con", ["head"]: run_loop($view$attrId$("welcome")), ["tail"]: { $: "Nil" } }, run_loop($chatWelcomeChildren$(connected_0, connecting_0))]);
}
