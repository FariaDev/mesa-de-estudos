...
function $slashItemId$(i_0) {
  const x_0 = run_loop($Nat$show$(i_0));
  return "slash-opt-" + x_0;
}
function $slashArgSlot$(hint_0) {
  return run_jump($view$viewWhen$, [run_loop($Bool$not$(run_loop($String$is_empty$(hint_0)))), run_loop($view$viewEl$("span", { $: "Con", ["head"]: run_loop($view$attrClass$("slash-arg")), ["tail"]: { $: "Nil" } }, { $: "Con", ["head"]: run_loop($view$viewText$(hint_0)), ["tail"]: { $: "Nil" } }))]);
}
function $slashNameNode$(name_0, hint_0) {
  return run_jump($view$viewEl$, ["span", { $: "Con", ["head"]: run_loop($view$attrClass$("slash-name")), ["tail"]: { $: "Nil" } }, run_loop($view$viewConcat$({ $: "Con", ["head"]: run_loop($view$viewText$("/" + name_0)), ["tail"]: { $: "Nil" } }, run_loop($slashArgSlot$(hint_0))))]);
}
function $slashDescSlot$(desc_0) {
  return run_jump($view$viewWhen$, [run_loop($Bool$not$(run_loop($String$is_empty$(desc_0)))), run_loop($view$viewEl$("span", { $: "Con", ["head"]: run_loop($view$attrClass$("slash-desc")), ["tail"]: { $: "Nil" } }, { $: "Con", ["head"]: run_loop($view$viewText$(desc_0)), ["tail"]: { $: "Nil" } }))]);
}
function $slashTextNode$(name_0, hint_0, desc_0) {
  return run_jump($view$viewEl$, ["div", { $: "Con", ["head"]: run_loop($view$attrClass$("slash-text")), ["tail"]: { $: "Nil" } }, run_loop($view$viewConcat$({ $: "Con", ["head"]: run_loop($slashNameNode$(name_0, hint_0)), ["tail"]: { $: "Nil" } }, run_loop($slashDescSlot$(desc_0))))]);
}
...
function $slashItemNode$(i_0, item_0, activeKey_0) {
  const key_0 = item_0.key;
  const name_0 = item_0.name;
  const hint_0 = item_0.hint;
  const desc_0 = item_0.desc;
  const key_1 = key_0;
  const active_0 = run_loop($String$eq$(key_1, activeKey_0));
  return run_jump($view$viewEl$, ["div", { $: "Con", ["head"]: run_loop($view$classes$({ $: "Con", ["head"]: run_loop($view$classOn$("slash-item", true)), ["tail"]: { $: "Con", ["head"]: run_loop($view$classOn$("active", active_0)), ["tail"]: { $: "Nil" } } })), ["tail"]: { $: "Con", ["head"]: run_loop($view$attrId$(run_loop($slashItemId$(i_0)))), ["tail"]: { $: "Con", ["head"]: run_loop($view$attrKey$(key_1)), ["tail"]: { $: "Con", ["head"]: run_loop($view$attr$("role", "option")), ["tail"]: { $: "Con", ["head"]: run_loop($view$attrAria$("selected", run_loop($view$boolStr$(active_0)))), ["tail"]: { $: "Con", ["head"]: run_loop($view$attrOn$("mousedown", "hold")), ["tail"]: { $: "Con", ["head"]: run_loop($view$attrOn$("mouseenter", "hover")), ["tail"]: { $: "Con", ["head"]: run_loop($view$attrOn$("click", "pick")), ["tail"]: { $: "Nil" } } } } } } } } }, { $: "Con", ["head"]: run_loop($slashTextNode$(name_0, hint_0, desc_0)), ["tail"]: { $: "Con", ["head"]: run_loop($slashCheckNode$()), ["tail"]: { $: "Nil" } } }]);
}
function $slashItemViews$go$(xs_0, activeKey_0, i_0) {
  if (xs_0.$ === "Nil") {
    return { $: "Nil" };
  } else {
    const h_0 = xs_0.head;
    const t_0 = xs_0.tail;
    return { $: "Con", ["head"]: run_loop($slashItemNode$(i_0, h_0, activeKey_0)), ["tail"]: run_loop($slashItemViews$go$(t_0, activeKey_0, nat_chk(i_0 + 1n))) };
  }
}
function $slashItemViews$(xs_0, activeKey_0) {
  return run_jump($slashItemViews$go$, [xs_0, activeKey_0, 0n]);
}
...
function $slashListNode$(items_0, activeKey_0) {
  return run_jump($view$viewEl$, ["div", { $: "Con", ["head"]: run_loop($view$attrId$("slash-list")), ["tail"]: { $: "Con", ["head"]: run_loop($view$attrClass$("slash-list")), ["tail"]: { $: "Con", ["head"]: run_loop($view$attr$("role", "listbox")), ["tail"]: { $: "Con", ["head"]: run_loop($view$attrAria$("label", run_loop($slashListLabel$()))), ["tail"]: { $: "Nil" } } } } }, run_loop($slashItemViews$(items_0, activeKey_0))]);
}
...
function $slashMenuView$(open_0, items_0, activeKey_0) {
  return run_jump($view$viewEl$, ["div", run_loop($view$attrJoin$({ $: "Con", ["head"]: { $: "Con", ["head"]: run_loop($view$attrId$("slash-menu")), ["tail"]: { $: "Con", ["head"]: run_loop($view$attrClass$("slash-pop")), ["tail"]: { $: "Con", ["head"]: run_loop($view$attr$("style", "position:fixed;z-index:105")), ["tail"]: { $: "Nil" } } } }, ["tail"]: { $: "Con", ["head"]: run_loop($view$attrWhen$(run_loop($Bool$not$(open_0)), run_loop($view$attr$("hidden", "")))), ["tail"]: { $: "Nil" } } })), { $: "Con", ["head"]: run_loop($slashHeadNode$()), ["tail"]: { $: "Con", ["head"]: run_loop($slashListNode$(items_0, activeKey_0)), ["tail"]: { $: "Con", ["head"]: run_loop($slashFootNode$()), ["tail"]: { $: "Nil" } } } }]);
}
