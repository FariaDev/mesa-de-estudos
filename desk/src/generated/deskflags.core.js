// GERADO de core/deskflags.bend por `bun core/build.mjs` — não editar à mão.
// core/deskflags.bend
...
function $hiddenAttrs$(_hidden_0) {
  return run_jump($view$attrWhen$, [_hidden_0, { $: "ViewAttr", ["name"]: "hidden", ["value"]: "" }]);
}
function $refsPressed$if$(_refsToggle_0, _includeRefs_0) {
  if (_refsToggle_0) {
    return _includeRefs_0;
  } else {
    return true;
  }
}
function $includeRefsButton$(_f_0) {
  const _refsToggle_0 = _f_0["refsToggle"];
  const _includeRefs_0 = _f_0["includeRefs"];
  const __0 = _f_0["studyContext"];
  const __1 = _f_0["conferir"];
  const __2 = _f_0["calculator"];
  return run_jump($view$viewEl$, ["button", run_loop($view$attrJoin$({ $: "Con", ["head"]: { $: "Con", ["head"]: run_loop($view$attrId$("include-refs")), ["tail"]: { $: "Con", ["head"]: run_loop($view$attrAria$("pressed", run_loop($view$boolStr$(run_loop($refsPressed$if$(_refsToggle_0, _includeRefs_0)))))), ["tail"]: { $: "Nil" } } }, ["tail"]: { $: "Con", ["head"]: run_loop($hiddenAttrs$(run_loop($Bool$not$(_refsToggle_0)))), ["tail"]: { $: "Nil" } } })), { $: "Nil" }]);
}
function $studyContextRow$(_f_0) {
  const __0 = _f_0["refsToggle"];
  const __1 = _f_0["includeRefs"];
  const _studyContext_0 = _f_0["studyContext"];
  const __2 = _f_0["conferir"];
  const __3 = _f_0["calculator"];
  return run_jump($view$viewEl$, ["div", run_loop($view$attrJoin$({ $: "Con", ["head"]: { $: "Con", ["head"]: run_loop($view$attrId$("study-context")), ["tail"]: { $: "Nil" } }, ["tail"]: { $: "Con", ["head"]: run_loop($hiddenAttrs$(run_loop($Bool$not$(_studyContext_0)))), ["tail"]: { $: "Nil" } } })), { $: "Nil" }]);
}
function $conferirButton$(_f_0) {
  const __0 = _f_0["refsToggle"];
  const __1 = _f_0["includeRefs"];
  const __2 = _f_0["studyContext"];
  const _conferir_0 = _f_0["conferir"];
  const __3 = _f_0["calculator"];
  return run_jump($view$viewEl$, ["button", run_loop($view$attrJoin$({ $: "Con", ["head"]: { $: "Con", ["head"]: run_loop($view$attrId$("check")), ["tail"]: { $: "Nil" } }, ["tail"]: { $: "Con", ["head"]: run_loop($hiddenAttrs$(run_loop($Bool$not$(_conferir_0)))), ["tail"]: { $: "Nil" } } })), { $: "Nil" }]);
}
function $calcDivider$(_f_0) {
  const __0 = _f_0["refsToggle"];
  const __1 = _f_0["includeRefs"];
  const __2 = _f_0["studyContext"];
  const __3 = _f_0["conferir"];
  const _calculator_0 = _f_0["calculator"];
  return run_jump($view$viewEl$, ["div", run_loop($view$attrJoin$({ $: "Con", ["head"]: { $: "Con", ["head"]: run_loop($view$attrId$("calc-divider")), ["tail"]: { $: "Nil" } }, ["tail"]: { $: "Con", ["head"]: run_loop($hiddenAttrs$(run_loop($Bool$not$(_calculator_0)))), ["tail"]: { $: "Nil" } } })), { $: "Nil" }]);
}
...
var deskflags_default = {
  ...
  hiddenAttrs: run_lib($hiddenAttrs$, 1),
  "refsPressed.if": run_lib($refsPressed$if$, 2),
  includeRefsButton: run_lib($includeRefsButton$, 1),
  studyContextRow: run_lib($studyContextRow$, 1),
  conferirButton: run_lib($conferirButton$, 1),
  calcDivider: run_lib($calcDivider$, 1)
};
export {
  deskflags_default as default
};
