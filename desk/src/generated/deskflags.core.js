...
function $hiddenAttrs$(hidden_0) {
  return run_jump($view$attrWhen$, [hidden_0, { $: "ViewAttr", ["name"]: "hidden", ["value"]: "" }]);
}
function $refsPressed$if$(refsToggle_0, includeRefs_0) {
  if (refsToggle_0) {
    return includeRefs_0;
  } else {
    return true;
  }
}
function $includeRefsButton$(f_0) {
  const refsToggle_0 = f_0.refsToggle;
  const includeRefs_0 = f_0.includeRefs;
  const __0 = f_0.endDay;
...
function $endDayButton$(f_0) {
  const __0 = f_0.refsToggle;
  const __1 = f_0.includeRefs;
  const endDay_0 = f_0.endDay;
...
var deskflags_default = {
  ...
  endDayButton: run_lib($endDayButton$, 1),
