...
function $quoteIsLong$drop$(_rest_0) {
  if (_rest_0 === "") {
    return false;
  } else {
...
function $quoteIsLong$(_s_0) {
  return run_jump($quoteIsLong$drop$, [run_loop($String$drop$(_s_0, run_loop($quoteChipMax$())))]);
}
function $quoteDisplay$given$(_flat_0, _long_0) {
  if (!_long_0) {
    return _flat_0;
  } else {
    const _x_0 = run_loop($quoteChipMax$());
    const _x_1 = run_loop($String$take$(_flat_0, _x_0 < 1n ? 0n : _x_0 - 1n));
    return _x_1 + "…";
  }
}
function $quoteDisplay$(_flat_0) {
  return run_jump($quoteDisplay$given$, [_flat_0, run_loop($quoteIsLong$(_flat_0))]);
}
function $quoteChipText$given$(_flat_0, _long_0) {
  return run_jump($String$join$, [{ $: "Con", ["head"]: "“", ["tail"]: { $: "Con", ["head"]: run_loop($quoteDisplay$given$(_flat_0, _long_0)), ["tail"]: { $: "Con", ["head"]: "”", ["tail"]: { $: "Nil" } } } }, ""]);
}
function $quoteChipText$(_flat_0) {
  return run_jump($quoteChipText$given$, [_flat_0, run_loop($quoteIsLong$(_flat_0))]);
}
function $lazyAttrs$(_lazy_0) {
  if (_lazy_0) {
...
function $removeTitleAttrs$if$(_title_0, _empty_0) {
  if (_empty_0) {
    return { $: "Nil" };
  } else {
...
function $removeTitleAttrs$(_title_0) {
  return run_jump($removeTitleAttrs$if$, [_title_0, run_loop($String$is_empty$(_title_0))]);
}
function $attachImg$(_dataUrl_0, _alt_0, _lazy_0) {
  return run_jump($view$viewEl$, ["img", run_loop($view$attrJoin$({ $: "Con", ["head"]: { $: "Con", ["head"]: run_loop($view$attr$("src", _dataUrl_0)), ["tail"]: { $: "Con", ["head"]: run_loop($view$attr$("alt", _alt_0)), ["tail"]: { $: "Nil" } } }, ["tail"]: { $: "Con", ["head"]: run_loop($lazyAttrs$(_lazy_0)), ["tail"]: { $: "Nil" } } })), { $: "Nil" }]);
}
function $attachRemove$(_handler_0, _label_0, _title_0) {
  return run_jump($view$viewEl$, ["button", run_loop($view$attrJoin$({ $: "Con", ["head"]: { $: "Con", ["head"]: run_loop($view$attrType$("button")), ["tail"]: { $: "Con", ["head"]: run_loop($view$attrClass$("attachment-remove")), ["tail"]: { $: "Con", ["head"]: run_loop($view$attrAria$("label", _label_0)), ["tail"]: { $: "Nil" } } } }, ["tail"]: { $: "Con", ["head"]: run_loop($removeTitleAttrs$(_title_0)), ["tail"]: { $: "Con", ["head"]: { $: "Con", ["head"]: run_loop($view$attrOn$("click", _handler_0)), ["tail"]: { $: "Nil" } }, ["tail"]: { $: "Nil" } } } })), { $: "Con", ["head"]: run_loop($view$viewText$("×")), ["tail"]: { $: "Nil" } }]);
}
function $imageItem$(_row_0) {
  const _key_0 = _row_0["key"];
  const _dataUrl_0 = _row_0["dataUrl"];
  const _alt_0 = _row_0["alt"];
  const _removeLabel_0 = _row_0["removeLabel"];
  const _removeTitle_0 = _row_0["removeTitle"];
  const _handler_0 = _row_0["handler"];
  const _lazy_0 = _row_0["lazy"];
  return run_jump($view$viewKey$, ["div", _key_0, { $: "Con", ["head"]: run_loop($view$attrClass$("attachment")), ["tail"]: { $: "Nil" } }, { $: "Con", ["head"]: run_loop($attachImg$(_dataUrl_0, _alt_0, _lazy_0)), ["tail"]: { $: "Con", ["head"]: run_loop($attachRemove$(_handler_0, _removeLabel_0, _removeTitle_0)), ["tail"]: { $: "Nil" } } }]);
}
function $fileItem$(_row_0) {
  const _key_0 = _row_0["key"];
  const _name_0 = _row_0["name"];
  const _removeLabel_0 = _row_0["removeLabel"];
  const _removeTitle_0 = _row_0["removeTitle"];
  const _handler_0 = _row_0["handler"];
  return run_jump($view$viewKey$, ["div", _key_0, { $: "Con", ["head"]: run_loop($view$attrClass$("attachment")), ["tail"]: { $: "Nil" } }, { $: "Con", ["head"]: run_loop($view$viewEl$("div", { $: "Con", ["head"]: run_loop($view$attrClass$("file")), ["tail"]: { $: "Nil" } }, { $: "Con", ["head"]: run_loop($view$viewText$(_name_0)), ["tail"]: { $: "Nil" } })), ["tail"]: { $: "Con", ["head"]: run_loop($attachRemove$(_handler_0, _removeLabel_0, _removeTitle_0)), ["tail"]: { $: "Nil" } } }]);
}
function $imageItemViews$(_xs_0) {
  return run_jump($view$viewMap$1$, [_xs_0]);
}
function $fileItemViews$(_xs_0) {
  return run_jump($view$viewMap$2$, [_xs_0]);
}
function $attachmentViews$(_images_0, _files_0) {
  return run_jump($view$viewJoin$, [{ $: "Con", ["head"]: run_loop($imageItemViews$(_images_0)), ["tail"]: { $: "Con", ["head"]: run_loop($fileItemViews$(_files_0)), ["tail"]: { $: "Nil" } } }]);
}
function $quoteChipNode$(_flat_0, _long_0) {
  return run_jump($view$viewEl$, ["div", { $: "Con", ["head"]: run_loop($view$attrId$("quote-chip")), ["tail"]: { $: "Con", ["head"]: run_loop($view$attrClass$("quote-chip")), ["tail"]: { $: "Nil" } } }, { $: "Con", ["head"]: run_loop($view$viewEl$("span", { $: "Con", ["head"]: run_loop($view$attrClass$("quote-chip-label")), ["tail"]: { $: "Nil" } }, { $: "Con", ["head"]: run_loop($view$viewText$(run_loop($quoteChipLabel$()))), ["tail"]: { $: "Nil" } })), ["tail"]: { $: "Con", ["head"]: run_loop($view$viewEl$("span", { $: "Con", ["head"]: run_loop($view$attrClass$("quote-chip-text")), ["tail"]: { $: "Con", ["head"]: run_loop($view$attrTitle$(_flat_0)), ["tail"]: { $: "Nil" } } }, { $: "Con", ["head"]: run_loop($view$viewText$(run_loop($quoteChipText$given$(_flat_0, _long_0)))), ["tail"]: { $: "Nil" } })), ["tail"]: { $: "Con", ["head"]: run_loop($view$viewEl$("button", { $: "Con", ["head"]: run_loop($view$attrType$("button")), ["tail"]: { $: "Con", ["head"]: run_loop($view$attrClass$("quote-chip-remove")), ["tail"]: { $: "Con", ["head"]: run_loop($view$attrAria$("label", run_loop($quoteChipRemove$()))), ["tail"]: { $: "Con", ["head"]: run_loop($view$attrTitle$(run_loop($quoteChipRemove$()))), ["tail"]: { $: "Con", ["head"]: run_loop($view$attrOn$("click", "RemoveQuote")), ["tail"]: { $: "Nil" } } } } } }, { $: "Con", ["head"]: run_loop($view$viewText$("×")), ["tail"]: { $: "Nil" } })), ["tail"]: { $: "Nil" } } } }]);
}
function $quoteChipView$given$(_flat_0, _long_0) {
  return run_jump($quoteChipNode$, [_flat_0, _long_0]);
}
function $quoteChipView$(_flat_0) {
  return run_jump($quoteChipNode$, [_flat_0, run_loop($quoteIsLong$(_flat_0))]);
}
...
function $view$viewMap$1$(_xs_0) {
  if (_xs_0.$ === "Nil") {
    ...
    const _h_0 = _xs_0["head"];
    const _t_0 = _xs_0["tail"];
    return { $: "Con", ["head"]: run_loop($imageItem$(_h_0)), ["tail"]: run_loop($view$viewMap$1$(_t_0)) };
  }
}
function $view$viewMap$2$(_xs_0) {
  if (_xs_0.$ === "Nil") {
    return { $: "Nil" };
  } else {
    const _h_0 = _xs_0["head"];
    const _t_0 = _xs_0["tail"];
    return { $: "Con", ["head"]: run_loop($fileItem$(_h_0)), ["tail"]: run_loop($view$viewMap$2$(_t_0)) };
  }
}
